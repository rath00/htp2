...
def detect(image_position):
    ...
    if classes[0]>=1 and classes[1]>=1 and classes[2]>=1:
        text=text+('這張圖上有一個有門、%d個窗戶和煙囪的房子\n'%classes[1])
        result=result+('性格開放、願意主動和外界交流，也願意讓別人去了解自己，另外還很在意家庭，')
        result_dc=result_dc+()
    elif classes[0]>=1 and classes[1]>=1 and classes[2]==0:
        text=text+('這張圖上有一個有門和%d個窗戶的房子\n'%classes[1])   # ，但沒有煙囪
        result=result+('性格開放、願意和外界交流，也願意讓別人去了解自身，但在家庭上可能缺乏溫暖，')
        result_dc=result_dc+()
    elif classes[0]>=1 and classes[1]==0 and classes[2]>=1:
        text=text+('這張圖上有一個有門和煙囪的房子\n')  # ，但沒有窗戶
        result=result+('性格開放，且願意和外界交流，但個性可能比較偏執和退縮，另外也很在意家庭，\n')
    elif classes[0]>=1 and classes[1]==0 and classes[2]==0:
        text=text+('這張圖上有一個只有門的房子\n')  # ，沒有窗戶和煙囪
        result=result+('性格開放，且願意和外界交流，但個性可能比較偏執和退縮，且缺乏家庭上的溫暖，\n')
    elif classes[0]==0 and classes[1]>=1 and classes[2]>=1:
        text=text+('這張圖上有一個有%d個窗戶和煙囪的房子\n'%classes[1])   # ，但沒有門
        result=result+('可能對外界有一點防禦心，但還是願意和外界交流，另外也很在意家庭，\n')
    elif classes[0]==0 and classes[1]>=1 and classes[2]==0:
        text=text+('這張圖上有一個有%d個窗戶的房子\n'%classes[1])    # ，但沒有門和煙囪
        result=result+('可能對外界有一點防禦心，在家庭上也缺乏一些溫暖，但還是願意和外界交流，讓別人瞭解自己，\n')
    elif classes[0]==0 and classes[1]==0 and classes[2]>=1:
        text=text+('這張圖上有一個只有煙囪的房子\n')  # ，沒有門和窗戶
        result=result+('性格比較封閉，且性格退縮和偏執，對外界有防禦心，不太願意和他人接觸，但是很在意家庭，\n')
    elif classes[0]==0 and classes[1]==0 and classes[2]==0:
        text=text+('這張圖上有一個只有外殼形狀的房子\n')
        result=result+('性格比較封閉，且性格退縮和偏執，對外界有防禦心，不太願意和他人接觸，缺乏家庭上的溫暖，\n')
   
    # classes=[0door,1window,2chimney,3face,4body,5fruit,6bark,7crown,8neck]  
    # 樹的結果
    result=result+('且此繪者')

    if classes[5]>=1 and classes[6]>=1 and classes[7]>=1:
        text=text+('還有一個有%d個果實和樹冠且樹幹上有樹紋的樹\n'%classes[5])
        result=result+('當前的事業或課業發展狀況佳，對於生活也有明確目標和成果，不過在過往的成長經歷上可能有遇到一些創傷，需要被注意，')
    elif classes[5]>=1 and classes[6]>=1 and classes[7]==0:
        text=text+('還有一個有%d個果實和樹紋的樹\n'%classes[5])     #，但沒有樹冠
        result=result+('缺乏自信、個性內向，但對於生活有明確的目標和成果，不過在過往的成長經歷上可能有遇到一些創傷，需要被注意，')
    elif classes[5]>=1 and classes[6]==0 and classes[7]>=1:
        text=text+('還有一個有%d個果實和樹冠的樹\n'%classes[5])   # ，但樹幹上沒有樹紋
        result=result+('當前的事業或課業發展狀況較佳，對於生活也有明確目標和成果，與他人接觸相處良好適應性也不錯，')
    elif classes[5]>=1 and classes[6]==0 and classes[7]==0:
        text=text+('還有一個只有%d個果實的樹\n'%classes[5]) # ，沒有樹紋和樹冠
        result=result+('缺乏自信、個性內向，但對於生活有明確的目標和成果。\n')
    elif classes[5]==0 and classes[6]>=1 and classes[7]>=1:
        text=text+('還有一個有樹冠和樹紋的樹\n')    # ，但沒有果實
        result=result+('當前在事業或課業發展狀況較佳，但對自己的評價不高，目前沒有什麼目標，且在過往的成長經歷上可能有遇到一些創傷。\n')
    elif classes[5]==0 and classes[6]>=1 and classes[7]==0:
        text=text+('還有一個樹幹上有樹紋的樹\n')    #，但沒有果實和樹冠
        result=result+('缺乏自信、個性內向，對自己的評價不高，目前沒有什麼目標，且在過往的成長經歷上可能有遇到一些創傷。\n')
    elif classes[5]==0 and classes[6]==0 and classes[7]>=1:
        text=text+('還有一個有樹冠的樹\n')  # ，但沒有果實和樹紋
        result=result+('當前的事業或課業發展狀況較佳，對自己的評價不高，目前沒有什麼目標，但與他人接觸相處良好適應性也不錯。\n')
    elif classes[5]==0 and classes[6]==0 and classes[7]==0:
        text=text+('還有一個沒有樹冠的樹\n') # 、果實和樹紋
        result=result+('缺乏自信、個性內向，對自己的評價不高，目前沒有什麼目標。\n')

    # classes=[0door,1window,2chimney,3face,4body,5fruit,6bark,7crown,8neck]
    # 人的結果
    result=result+('；最後「人」則是最接近自身的象徵，表示個體的自我意識和人格的完整性，'
                    +'由圖畫中人的特徵可以得知，')

    if classes[3]>=1 and classes[4]>=1 and classes[8]>=1:
        text=text+('跟一個臉上有表情且有脖子，但身體是火柴人的人\n')
        result=result+('受測者本身願意和外界交流，較能用理性的角度去控制情感，但對此測驗可能抱有警戒心，或是對自我的概念不太明確，不願暴露自己的內心。\n')
    elif classes[3]>=1 and classes[4]>=1 and classes[8]==0:
        text=text+('跟一個臉上有表情，且身體是火柴人的人\n')   # 但沒有脖子
        result=result+('受測者本身願意和外界交流，但個性衝動有憑藉著本能行事的傾向，另外受測者對於此測驗可能抱有警戒心，或是對自我的概念不太明確，不願暴露自己的內心。\n')
    elif classes[3]>=1 and classes[4]==0 and classes[8]>=1:
        text=text+('跟一個臉上有表情且有脖子的人\n')
        result=result+('受測者本身願意和外界交流，也較能用理性的角度去控制情感。\n')
    elif classes[3]>=1 and classes[4]==0 and classes[8]==0:
        text=text+('跟一個臉上有表情的人\n')    # 但沒有脖子
        result=result+('受測者本身願意和外界交流，但個性衝動有憑藉著本能行事的傾向。')
    elif classes[3]==0 and classes[4]>=1 and classes[8]>=1:
        text=text+('跟一個有脖子，且身體是火柴人的人\n')  # 但臉上沒有表情
        result=result+('受測者本身逃避人際關係，沒有辦法很好的適應環境，另外受測者對於此測驗可能抱有警戒心，或是對自我的概念不太明確，不願暴露自己的內心。\n')
    elif classes[3]==0 and classes[4]>=1 and classes[8]==0:
        text=text+('跟一個身體是火柴人的人\n')  # 臉上沒有表情也沒有脖子，且
        result=result+('受測者本身逃避人際關係，沒有辦法很好的適應環境，且個性衝動有憑藉著本能行事的傾向，另外受測者對於此測驗可能抱有警戒心，或是對自我的概念不太明確，不願暴露自己的內心。\n')
    elif classes[3]==0 and classes[4]==0 and classes[8]>=1:
        text=text+('跟一個有脖子的人\n')    #臉上沒有表情但
        result=result+('受測者本身逃避人際關係，沒有辦法很好的適應環境，不過本身較能用理性的角度去控制情感，不會衝動行事。\n')
    elif classes[3]==0 and classes[4]==0 and classes[8]==0:
        text=text+('跟一個臉上沒有表情的人\n')  #也沒有脖子
        result=result+('受測者本身逃避人際關係，沒有辦法很好的適應環境，且個性衝動有憑藉著本能行事的傾向。')
    '''
